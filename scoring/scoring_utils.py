...
TRIAL_LINE_REGEX = r'(.*) --- Tuning run (\d+)/(\d+) ---'
METRICS_LINE_REGEX = '(.*) Metrics: ({.*})'
TRIAL_DIR_REGEX = r'trial_(\d+)'
MEASUREMENTS_FILENAME = 'eval_measurements.csv'
TIMESTAMP = r'-\d{4}(-\d{2}){5}'

# Per-workload scoring targets and the base/held-out workload sets are
# version-specific and live in a typed `WorkloadConfig` loaded from a
# `workload_targets*.json` file (see scoring/config.py). They are passed
# explicitly into the scoring functions rather than read from module globals.
